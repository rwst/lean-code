...
`lean/CollatzEndpointTransport/Common/QuantitativeDensity.lean` (Lean 4.15.0).
Adaptation to Lean 4.32.0-rc1 by Ralf Stephan in collaboration with Claude Code:
namespace `CollatzEndpointTransport` → `CET`, wholesale `import Mathlib` replaced
by fine-grained imports.  Upstream Apache-2.0 header retained.
-/
