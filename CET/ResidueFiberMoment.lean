...
`lean/CollatzEndpointTransport/Linear/FixedTotalQuotientOrder.lean` (Lean 4.15.0).
...
`CET/ResidueFiberMoment.lean`.  Upstream Apache-2.0 header retained.
-/
import CET.SyracuseComposition
...
theorem sum_residueFiber
  ...
  calc
    (∑ a : Fin m, if A x % m = (a : ℕ) then f x else 0) =
      ...
      · have ha : a₀ ≠ a := by
          intro ha
          exact hres (congrArg Fin.val ha)
        simp [hres, ha]
    _ = f x := by
      simp
