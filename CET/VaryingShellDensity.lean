...
`lean/CollatzEndpointTransport/Common/VaryingShellDensity.lean` (Lean 4.15.0).
Adaptation to Lean 4.32.0-rc1 by Ralf Stephan in collaboration with Claude Code:
namespace `CollatzEndpointTransport` → `CET`.  Upstream Apache-2.0 header retained.
-/
import CET.ShellToGlobalDensity
          ...
          ≤ ∑ M ∈ Finset.range M₀, (2 : ℝ) ^ M := by
            ...
            have hcard :
              ...
              calc
                ...
                _ = 2 ^ M := by
                      simp [dyadicShell, Nat.card_Ico, pow_succ]
                      omega
