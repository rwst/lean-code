...
`lean/CollatzEndpointTransport/Linear/FixedTotalComposition.lean` (Lean 4.15.0).
Adaptation to Lean 4.32.0-rc1 by Ralf Stephan in collaboration with Claude Code:
namespace `CollatzEndpointTransport` → `CET`, file renamed to
`CET/SyracuseComposition.lean`.  Upstream Apache-2.0 header retained.
-/
...
theorem syracuseNumeratorList_injective_of_length_sum
  ...
  | zero =>
      ...
      have hksNil : ks = [] := List.eq_nil_of_length_eq_zero hks
      have hlsNil : ls = [] := List.eq_nil_of_length_eq_zero hls
      simp [hksNil, hlsNil]
  | succ s ih =>
      ...
      · have hktZero : kt.length = 0 := hktLen.trans hs0
        ...
        have hktNil : kt = [] := List.eq_nil_of_length_eq_zero hktZero
        have hltNil : lt = [] := List.eq_nil_of_length_eq_zero hltZero
        subst kt
        subst lt
